        ...
        while (i < s.length) {
            ...
            if (ch == '*') {
                ...
                if (needMultiplicationSign(mathTypeBefore?.type, mathTypeAfter?.type)) {
                    // Check if Engine has multiplicationSign and it has a value, otherwise defaults to *
                    // Assuming engine.multiplicationSign is implemented as Flow/State or similar. 
                    // Need to verify Engine property structure. Using default "*" for now to be safe if property missing.
                    append("*") 
                }
            } else {
                when (mathType.type) {
