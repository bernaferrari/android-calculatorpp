 ...
 * Time: 11:09 AM
 */
class MeanSquareDeviation : AbstractStatFunction {
    ...
    override fun newInstance(parameters: Array<Generic>): Operator {
        return MeanSquareDeviation(parameters)
    }
        ...
        return if (parameters!![0] is JsclVector) {
            ...
            when {
                ...
                elements.size == 1 -> NumericWrapper(JsclInteger.ZERO)
                    ...
                    Sqrt(result.divide(JsclInteger.valueOf(elements.size.toLong()).numeric())).numeric()
                }
            }
        } else {
            expressionValue()
    ...
    override fun newInstance(): Variable {
        return MeanSquareDeviation(null as JsclVector?)
    }

    companion object {
        const val NAME = "mean_sq_dev"
    }
}
