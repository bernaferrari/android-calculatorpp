...
open class Expression : Generic {
    ...
    override fun add(that: Generic): Generic {
        val nonFinite = nonFiniteBinaryResult(that) { left, right -> left + right }
        if (nonFinite != null) {
            return nonFinite
        }
        return when (that) {
    ...
    override fun simplify(): Generic {
        val hasNonFiniteLiteral = containsNonFiniteLiteral()
        val numeric = try {
            numeric()
        } catch (_: Exception) {
            null
        ...
        if (numeric is NumericWrapper) {
            ...
            if (content is jscl.math.numeric.Real) {
                val value = content.doubleValue()
                if (value.isNaN() || value.isInfinite() || hasNonFiniteLiteral) {
                    return numeric
                }
            }
        }
        if (hasNonFiniteLiteral && numeric != null) {
            return numeric
        }
        if (hasNonFiniteLiteral) {
            return this
        }
        ...
        return when {
            ...
            size == 1 -> {
                ...
                if (k.compareTo(JsclInteger.valueOf(1)) == 0) {
                    ...
                    productElements.copyInto(result, 1, 0, 0 + productElements.size)
                    result[0] = k
    ...
    private fun nonFiniteBinaryResult(that: Generic, op: (Double, Double) -> Double): NumericWrapper? {
        val left = numericDoubleOrNull(this)
        val right = numericDoubleOrNull(that)

        if (left != null && left.isNaN()) {
            return NumericWrapper.valueOf(Double.NaN) as NumericWrapper
        }
        ...
        if (left.isInfinite() || right.isInfinite()) {
            return NumericWrapper.valueOf(result) as NumericWrapper
        }
        return if (result.isNaN() || result.isInfinite()) {
            NumericWrapper.valueOf(result) as NumericWrapper
        } else {
            null
        }
    }
    ...
    private fun containsNonFiniteLiteral(): Boolean {
        ...
        for (literal in literalsSnapshot) {
            ...
            for (variable in literal.variables()) {
                ...
                val wrapper = variableContent as? NumericWrapper ?: continue
                val real = wrapper.content() as? jscl.math.numeric.Real ?: continue
                val value = real.doubleValue()
