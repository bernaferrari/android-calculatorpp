package jscl.math.function

import jscl.AngleUnit
import jscl.JsclMathEngine
import jscl.math.Generic
import jscl.math.NotIntegrableException
import jscl.math.Variable
import jscl.math.polynomial.Polynomial
import jscl.text.msg.Messages

abstract class Trigonometric(name: String, parameter: Array<Generic>?) : Function(name, parameter) {

    override fun antiDerivative(variable: Variable): Generic {
        if (JsclMathEngine.getInstance().getAngleUnits() != AngleUnit.rad) {
            throw NotIntegrableException(Messages.msg_20, name)
        }

        val parameter = parameters!![0]
        if (parameter.isPolynomial(variable)) {
            val polynomial = Polynomial.factory(variable).valueOf(parameter)
            if (polynomial.degree() == 1) {
                val elements = polynomial.elements()
                return Inverse(elements[1]).selfExpand().multiply(antiDerivative(0))
            } else {
                throw NotIntegrableException(this)
            }
        } else {
            throw NotIntegrableException(this)
        }
    }

    fun identity(): Generic {
//      Generic a[]=parameter[0].sumValue();
//      if(a.length>1) {
//          Generic s=JsclInteger.valueOf(0);
//          for(int i=1;i<a.length;i++) s=s.add(a[i]);
//          return identity(a[0],s);
//      }
//      Generic n[]=Frac.separateCoefficient(parameter[0]);
//      if(n[0].compareTo(JsclInteger.valueOf(1))==0);
//      else {
//          Generic s=new Frac(n[2],n[1]).evaluateSimplify();
//          return identity(s,n[0].subtract(JsclInteger.valueOf(1)).multiply(s));
//      }
        return expressionValue()
    }

    abstract fun identity(a: Generic, b: Generic): Generic
}
