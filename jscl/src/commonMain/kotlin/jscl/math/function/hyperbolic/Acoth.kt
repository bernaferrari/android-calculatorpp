...
open class Acoth(generic: Generic?) : ArcTrigonometric("acoth", generic?.let { arrayOf(it) }) {
    ...
    override fun selfExpand(): Generic {
        if (parameters!![0].signum() < 0) {
            return Acoth(parameters!![0].negate()).selfExpand().negate()
        }
    ...
    override fun selfNumeric(): Generic {
        return (parameters!![0] as NumericWrapper).acoth()
    }
