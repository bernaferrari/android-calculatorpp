...
enum class MathType(
    ...
    open fun getTokens(): List<String> {
        return tokens
    }
        ...
        fun getType(text: String, i: Int, hexMode: Boolean, result: Result, engine: Engine): Result {
            ...
            for (mathType in mathTypes) {
                ...
                if (s.length > 1) {
                    if (mathType == function) {
                        ...
                        if (nextToken < text.length) {
                            ...
                            if (open_group_symbol.getTokens().contains(text.substring(nextToken, nextToken + 1))) {
                ...
                if (mathType == grouping_separator) {
                    if (i + 1 < text.length &&
                        digit.getTokens().contains(text.substring(i + 1, i + 2)) &&
                        i - 1 >= 0 && digit.getTokens().contains(text.substring(i - 1, i))
                    ) {
                        return result.set(mathType, s)
                    }
                    continue
